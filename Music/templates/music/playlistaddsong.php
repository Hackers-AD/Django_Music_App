...
<script type="text/javascript">
	function ShowSongs(argument) {
		if (argument.value=="Only Playlist Songs") {
		...
		if (argument.value=="Only Songs not in Playlist") {
			list=document.getElementsByTagName('li');
			for (var i = 0; i < list.length; i++) {
				input=list[i].getElementsByTagName('input');
				console.log(input[0].value);
...
<body>
	<header>
		...
		<div>
			...
			<b style="color: #340000">{{playlist.name}}</b>
	    </div>
	...
	<div class="navbar navbar-default">
		<h3><span class="glyphicon glyphicon-plus-sign"></span> Add Song In My Playlist</h3>
	</div>
	<p id="addsonginfo"></p>
	<select style="float: right;border: 1px solid grey;border-radius: 4px 4px;" onchange="ShowSongs(this)">
		<option>All Songs</option>
		<option>Only Playlist Songs</option>
		<option>Only Songs not in Playlist</option>
	</select>
	{%if playlistacess%}
	    <section id="addsong"><ol>
	    	...
	    	<form action="" method="post">{%csrf_token%}
	    		...
	    		{%if song|is_in_playlist:playlist_id%}
	...
	{%else%}
	  <center>You have no permission to change a song to this Playlist.</center>
	{%endif%}
	<footer>
